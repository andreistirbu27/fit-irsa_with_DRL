...
input_dim = input_obs_dim + feedback_dim + user_status_dim
hidden_dim = 128
epochs = 2000
batch_size = 50
learning_rate = 0.02
...
class PolicyNetUser(nn.Module):
    def __init__(self):
        ...
        # optional: start slightly sparse
        with torch.no_grad():
            last = self.net[-1]
            if isinstance(last, nn.Linear):
                last.bias.fill_(-1.4)  # sigmoid(-1.4) ~ 0.20
...
def feedback_indices_to_vector(feedback_indices):
    ...
    d = torch.zeros(num_slots); e = torch.zeros(num_slots); u = torch.zeros(num_slots)
...
def train(policy, optimizer,
    ...
    """
    Per-user policy, 2 rounds with feedback.
    Adds ramped sparsity encouragement in both rounds.
    Tracks: frac(R1-decoded who transmit in R2).
    I have:
      - num_users, num_slots, input_obs_dim, epochs, batch_size
      - sample_actions_user, run_sic_simulation(return_feedback_indices=True), feedback_indices_to_vector
    """
    reward_history = []
    avg_unique_history = []
    frac_decR1_txR2_hist = []   # epoch-level metric you asked for

    for epoch in tqdm(range(epochs), desc="Training", unit="epoch"):
        # ramp sparsity over time (curriculum)
        ...
        for _ in range(batch_size):
            ...
            # ---------- Round 1: per-user policy (feedback=0, status=1) ----------
            actions_r1, acts_bin_r1 = [], []
            lp_r1_total = 0.0
            for u in range(num_users):
                x = torch.cat([obs_all[u],
                               torch.zeros(3 * num_slots),
                               torch.tensor([1.0])], dim=0)        # status=1
            ...
            # ---------- Round 2: per-user policy (feedback + per-user status) ----------
            fb_vec = feedback_indices_to_vector(fb_idx)            # length = 3*num_slots
            actions_r2, acts_bin_r2 = [], []
            lp_r2_total = 0.0
            for u in range(num_users):
                status = 0.0 if u in decoded_r1 else 1.0           # 1 = still needs decoding
                x = torch.cat([obs_all[u], fb_vec, torch.tensor([status])], dim=0)
                logits_u = policy(x)
            ...
            # ---------- Ramped sparsity (both rounds) ----------
            r1_activity = acts_bin_r1.float().mean().item()        # ∈ [0,1]
            r2_activity = acts_bin_r2.float().mean().item()        # ∈ [0,1]
            reward -= lam_r1 * r1_activity
            reward -= lam_r2 * r2_activity
            last_r1_act, last_r2_act = r1_activity, r2_activity

            # ---------- Your metric: frac(R1-decoded who transmit in R2) ----------
            if len(decoded_r1) > 0:
                mask_dec = torch.zeros(num_users, dtype=torch.bool)
                mask_dec[list(decoded_r1)] = True
                r2_any = (acts_bin_r2.sum(dim=1) > 0)              # [num_users] boolean
                frac_decR1_txR2 = r2_any[mask_dec].float().mean().item()
            else:
                frac_decR1_txR2 = float('nan')                     # no R1 decodes this episode
        ...
        epoch_frac = np.nanmean(batch_frac_decR1_txR2)   # ignore episodes with no R1 decodes
        frac_decR1_txR2_hist.append(epoch_frac)

        if epoch % 100 == 0:
            print(f"Epoch {epoch}: "
                  f"Avg Reward={baseline:.3f}, "
                  f"Avg unique={avg_unique_history[-1]:.2f}/{num_users}, "
                  f"frac(R1-decoded tx in R2)={epoch_frac:.3f}, "
