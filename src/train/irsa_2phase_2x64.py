...
def make_result_dir(cfg):
    ...
    if cfg['result_dir'] is not None:
        ...
        if cfg.get('num_layers', DEFAULT_NUM_LAYERS) != DEFAULT_NUM_LAYERS:
            parts.append(f"L{cfg['num_layers']}")
...
class PolicyNetUser(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_slots, num_layers=DEFAULT_NUM_LAYERS):
        ...
        for _ in range(num_layers):
            ...
            in_dim = hidden_dim
        layers += [nn.Linear(in_dim, num_slots)]   # logits per slot for THIS user
        self.net = nn.Sequential(*layers)
        with torch.no_grad():
            last = self.net[-1]
            if isinstance(last, nn.Linear):
...
def main():
    ...
    # === Run ===
    policy = PolicyNetUser(input_dim, cfg['hidden_dim'], cfg['num_slots'], cfg['num_layers'])
